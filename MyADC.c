...
void MyADC_Conf(void (*IT_function)(void)){
	...
	LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_USART1);//Enable clock USART1	
	LL_USART_Init(USART1, &USART_InitStruct);
	LL_USART_Enable(USART1);
	LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_GPIOA);	
	LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_11, LL_GPIO_MODE_OUTPUT);//Emetteur
	LL_GPIO_SetPinOutputType(GPIOA,LL_GPIO_PIN_11, LL_GPIO_OUTPUT_PUSHPULL);
	LL_GPIO_SetOutputPin(GPIOA, LL_GPIO_PIN_11);
	LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_9, LL_GPIO_MODE_OUTPUT);//USART1_TX->PA9
